...
typedef struct {
  ...
  ec_public_key_t tumbler_ec_pk;  // 在PuzzlePromise阶段由bob调用PreVf时使用
  ...
  schnorr_signature_t sigma_r;  // bob的有效签名
  schnorr_signature_t sigma_t;  // tumbler的有效签名
  bn_t beta;
  bn_t tid;
...
#define bob_state_new(state)                                \
  do {                                                      \
    ...
    schnorr_signature_new((state)->sigma_t);                \
    bn_new((state)->beta);                                  \
    bn_new((state)->tid);                                   \
...
#define bob_state_free(state)                               \
  do {                                                      \
    ...
    schnorr_signature_free((state)->sigma_t);               \
    bn_free((state)->beta);                                 \
    bn_free((state)->tid);                                  \
